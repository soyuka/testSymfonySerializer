...
namespace Acme\UserBundle\View;
 ...
 */
class ViewHandler extends ContainerAware implements \FOS\RestBundle\View\ConfigurableViewHandlerInterface
{
    /**
    ...
    private function initResponse(View $view, $format)
    {
        ...
        if ($this->isFormatTemplating($format)) {
            ...
            } else if($serializer instanceof SymfonySerializerInterface) {
                $context = $this->getSerializationContext($view);
                $groups = $context->attributes->get('groups')->getIterator();
                foreach($groups as $group) {
                    $this->exclusionStrategyGroups[] = $group[0];
                }

                // comment out to check
                // dump($this->exclusionStrategyGroups);
                // die();
                $content = $serializer->serialize($data, $format, ['groups' => $this->exclusionStrategyGroups]);
            } else {
